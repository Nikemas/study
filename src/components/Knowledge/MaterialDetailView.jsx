...
export const MaterialDetailView = ({ material, onClose, onProgressChange }) => {
  ...
  const welcomeMessage = useMemo(() => ({
    ...
    role: 'assistant',
    content: language === 'en'
      ? `Hi! I'll help you study the topic **"${material.topic}"**. Ask any questions about this topic Ч I'll explain in detail!`
      : language === 'ky'
        ? `—аламат! —ен **"${material.topic}"** темасын ?йр?н??г? жардамчы болом. Ѕул тема боюнча каалаган суроо бер Ч т?ш?нд?р?м!`
        : `ѕривет! я помогу тебе изучить тему **"${material.topic}"**. «адавай любые вопросы по этой теме Ч объ€сню подробно!`,
    timestamp: new Date()
                  ...
                  <ul className="space-y-2">
                    {material.keyPoints.map((point, idx) => (
                      <li key={idx} className={`flex items-start gap-2 text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                        <span className={`mt-0.5 ${config.colorClass}`}>Х</span>
                        {point}
                      </li>
                    ))}
                  </ul>
